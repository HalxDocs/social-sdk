...
See [tenant authorization](/docs/concepts/tenant-authorization) and the [account ownership diagram](/docs/concepts/architecture#account-connection-and-ownership) for the request boundary.
...
The [integration skill](/docs/agents/integrate-social-sdk) includes an executable handler recipe. Its provider and storage are injected by the application.
...
The server package includes direct OAuth factories for YouTube, X, TikTok, Threads, Instagram Login, and LinkedIn. Create them only in server code. Each factory accepts an injected `fetch` for deterministic tests, an optional `credentialSink`, and provider credentials. The `redirectUri` passed to the factory must match the exact callback URI registered with the provider and with `ConnectionManager.begin`.

| Provider        | Default authorization scopes                                     | PKCE                            | Token lifecycle                                                                                                                                             |
| --------------- | ---------------------------------------------------------------- | ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| YouTube         | `openid`, profile, `youtube.readonly`                            | Provider flow does not add PKCE | Google refresh tokens use `refreshOAuthToken("youtube", ...)`; request `youtube.upload` when uploading videos.                                              |
| X               | `tweet.read`, `users.read`, `offline.access`                     | S256                            | Refresh tokens can rotate; persist the returned replacement atomically.                                                                                     |
| TikTok          | `user.info.basic`, `video.publish`                               | S256                            | Refresh tokens can rotate; scopes are sent comma-delimited.                                                                                                 |
| Threads         | `threads_basic`, `threads_content_publish`                       | No fabricated PKCE              | Call `exchangeLongLivedOAuthToken("threads", ...)`, then `refreshOAuthToken("threads", ...)`.                                                               |
| Instagram Login | `instagram_business_basic`, `instagram_business_content_publish` | No fabricated PKCE              | Call `exchangeLongLivedOAuthToken("instagram", ...)`, then `refreshOAuthToken("instagram", ...)`.                                                           |
| LinkedIn        | `openid`, `profile`, `w_member_social`                           | No fabricated PKCE              | Refresh is not exposed by LinkedIn's authorization-code response; reconnect when the grant is unavailable. Set `linkedinApiVersion` explicitly as `YYYYMM`. |

The defaults are a starting point. Pass `scopes` for the operation you are connecting, such as Google's `https://www.googleapis.com/auth/youtube.upload` for uploads or LinkedIn's organization permission when your app has that product approval. Provider app review, account eligibility, callback registration, and quota remain application setup responsibilities.
