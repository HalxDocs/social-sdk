...
Persist the inbox state and the application effect together where possible. A worker may crash after acceptance; replay pending events and make the effect idempotent. Never trust a payload tenant field, assign an unmapped account to a default tenant, or log credentials, signatures, message content, or private account data. See the [authentication guide](/docs/authentication), [capability matrix](/docs/reference/capabilities), and [integration checklist](/docs/agents/integration-checklist).
