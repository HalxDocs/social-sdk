---
title: Content
description: Understand shared content, per-target options, formats, and local preparation in Social SDK.
---
...
Preparation applies each platform's actual rules: X's weighted 280-character counting, Bluesky's grapheme lengths, byte-offset mentions, and four-image cap, Threads' ten-item carousels, YouTube's title bounds. Failures return structured issues naming the field and rule, before the platform is contacted.
