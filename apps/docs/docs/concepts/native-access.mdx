...
const thread = await native.getThread(account, { uri, cid });
```

Selected reactions also ship as credential-ready helpers importable straight from a platform entrypoint, such as `xLike` and `xUnlike`, for server code that manages its own credentials.
