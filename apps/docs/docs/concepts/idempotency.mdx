...
description: Use idempotency keys, fingerprints, and bounded retry budgets so at-least-once execution never double-posts.
---
...
Job runners with at-least-once semantics pair well with this model: the runner may re-execute, and the key makes re-execution harmless. See the [Trigger.dev](/integrations/trigger-dev) and [Inngest](/integrations/inngest) patterns.
