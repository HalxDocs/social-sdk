...
<Steps>
  <Step title="Complete Meta app setup">
    Create a Meta app with Threads API access and complete the app review your use case requires. Token, user ID, scopes, and account eligibility remain server-owned setup.
  </Step>
  <Step title="Store credentials server-side">
    Set `THREADS_USER_ID` and `THREADS_ACCESS_TOKEN` in server configuration, and `THREADS_GRAPH_VERSION` only when you need a non-default version.
  </Step>
