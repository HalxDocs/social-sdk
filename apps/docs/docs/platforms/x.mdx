...
video or GIF per post. The adapter polls `processing_info` honoring `check_after_secs` within
the operation budget; a `failed` processing state or an over-duration `403` on attach surfaces as a
terminal `media_error` and never creates a post. Reconcile `timeout`/`unknown` outcomes before
retrying.
