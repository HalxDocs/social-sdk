...
X access depends on the account's current API plan and OAuth2 scopes, including `tweet.read`, `tweet.write`, `users.read`, `offline.access`, and `media.write` for the implemented operations. API usage can incur upstream charges. Use the documented [connection flow](/authentication) for callbacks and token persistence. API plan access, billing, and app approval remain upstream requirements. Keep access tokens server-side and supply an application authorization policy before publishing for multiple tenants.
