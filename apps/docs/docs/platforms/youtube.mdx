...
<Steps>
  <Step title="Configure Google OAuth">
    ...
    videos. Add `youtube.force-ssl` for captions and playlist mutations, and `yt-analytics.readonly`
    for Analytics API reports. Refresh with `refreshOAuthToken("youtube", ...)` and persist any
