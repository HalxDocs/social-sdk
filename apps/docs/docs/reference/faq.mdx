...
The SDK exposes scheduling operations where a backend supports them, and it never runs background jobs itself. Durable timing belongs to your application through a job runner such as [Trigger.dev](/integrations/trigger-dev) or [Inngest](/integrations/inngest).
