...
Convex applications call Social SDK from actions running in the Node runtime, and keep the durable state the SDK deliberately does not own — connections, publication records, delivery references, and the webhook inbox — in Convex tables.
